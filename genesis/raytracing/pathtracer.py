...
class RayTracer:
    ...
    def trace(self):
        ...
        # Fix: Improved distance handling logic to avoid empty point clouds
        # Use a more reasonable distance threshold instead of simply 9999
        max_reasonable_distance = 50.0  # 50 meters as a reasonable upper limit
        t[t > max_reasonable_distance] = 0
        
        # Add: Filter out invalid depth values
        t[t < 0.01] = 0  # Filter out points that are too close (may be noise)
        
        distance = np.array(t).reshape(self.PIR_resolution,self.PIR_resolution)
        intensity = np.array(intensity)[:,:,0]
        
        # Fix: Improved intensity handling, keep consistent with distance
        # Set intensity to 0 where distance is invalid
        intensity[distance == 0] = 0
        ...
        pointclouds = np.array(si.p)        # We save the points here for faster calculation, it can be calculated from the PIR's distance + sensor's intrinsic matrix
        
        # Fix: Filter point clouds, keep only valid points
        valid_mask = distance.flatten() > 0.01
        if valid_mask.any():
            pointclouds = pointclouds[valid_mask]
        else:
            pointclouds = np.array([]).reshape(0, 3)
