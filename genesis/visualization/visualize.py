...
def draw_combined(i,pointcloud_cfg,radar_frames,pointclouds,smpl_data):
    ...
    # Modified: Change from 3 columns to 4 columns layout, add front view
    fig= plt.figure(figsize=(16, 6))  # Increase width to fit 4 columns
    ...
    # Column 2: Point cloud (side view)
    ax2 = fig.add_subplot(142, projection='3d')
    draw_poinclouds_on_axis(pointclouds[radar_frame_id],ax2, None,None,30,-30,"Point clouds (Side View)")

    # Column 3: Point cloud (front view)
    ax3 = fig.add_subplot(143, projection='3d')
    # Front view: look from positive Z axis (elev=0, azim=90)
    draw_poinclouds_on_axis(pointclouds[radar_frame_id],ax3, None,None,0,90,"Point clouds (Front View)")

    # Column 4: Doppler plot
    ax4 = fig.add_subplot(144)
    draw_doppler_on_axis(radar_frames[radar_frame_id],pointcloud_cfg, ax4)
...
def save_video(radar_cfg_file, radar_frames_file, smpl_data_file, output_file):
    ...
    # Modified: Update video size to fit 4 columns layout
    out = cv2.VideoWriter(video_filename, fourcc, 30, (1600, 600))  # Width changed from 1200 to 1600
